...
class ViewModelFactory(private val authRepository: RegisterRepository, private val historyRepository: HistoryRepository, private val predictRepository: PredictRepository, private val hospitalRepository: HospitalRepository) : ViewModelProvider.NewInstanceFactory() {
        ...
        fun getInstance(context: Context): ViewModelFactory {
            if (INSTANCE == null) {
                synchronized(ViewModelFactory::class.java) {
                    INSTANCE = ViewModelFactory(Injection.registerInjection(context), Injection.historyInjection(context), Injection.predictInjection(context), Injection.hospitalInjection(context))
                }
            }
